...
class TicTacToeUI:
    def __init__(self, game=None, size=750):
        ...
        # Pieces
        self.X_IMAGE = load_piece_image("x_piece.png", "x.png", (self.size//10, self.size//10))
        self.O_IMAGE = load_piece_image("o_piece.png", "o.png", (self.size//10, self.size//10))
    ...
    def draw_banner(self):
        # Card HUD centered in top-left quadrant
        ...
        if winner is not None:
            ...
            badge_img = self.X_IMAGE if winner == 'x' else self.O_IMAGE
        elif is_draw:
            ...
            badge_img = self.X_IMAGE if curr_turn_piece == 'x' else self.O_IMAGE

        # Draw card background & border
        pygame.draw.rect(self.screen, self.white, card_rect, border_radius=16)
        ...
        self.screen.blit(lbl_title, lbl_title.get_rect(center=(card_rect.centerx, card_rect.top + 32)))

        # Draw Status Description & Badge Icon
        lbl_desc = self.font_small.render(status_desc, True, text_color)
        if badge_img:
            icon_scaled = pygame.transform.scale(badge_img, (38, 38))
            total_w = lbl_desc.get_width() + 48
            start_x = card_rect.centerx - (total_w // 2)
            self.screen.blit(icon_scaled, (start_x, card_rect.top + 80))
            self.screen.blit(lbl_desc, (start_x + 48, card_rect.top + 90))
        else:
            self.screen.blit(lbl_desc, lbl_desc.get_rect(center=(card_rect.centerx, card_rect.top + 95)))
    ...
    def draw_current_board(self):
        ...
        for index in range(9):
            ...
            if space is not None:
                ...
                img = self.X_IMAGE if space == 'x' else self.O_IMAGE
        ...
        for indx in range(6):
            ...
            if letter in pieces:
                ...
                img = self.X_IMAGE if arm.piece == 'x' else self.O_IMAGE
                img_rect = img.get_rect(center=rect.center)
                self.screen.blit(img, img_rect)
    ...
    def draw_button(self, rect, text, is_hovered, font=None):
        ...
        if lbl_test.get_width() > rect.width - 20:
    ...
    def show_setup_menu(self):
        ...
        lvl = 0
        btn_width, btn_height = int(self.WIDTH * 0.85), int(self.HEIGHT * 0.08)
        ...
        while selecting_diff:
            ...
            title = self.font_pieces.render("TicTacToe (RPi Edition)", True, self.blk)
            ...
            any_hovered = False
            for rect, text, level in [(btn_novice, "0: Novice (Default)", 0), (btn_pro, "1: Pro", 1), (btn_expert, "2: Expert", 2)]:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key in (pygame.K_0, pygame.K_KP0, pygame.K_RETURN, pygame.K_KP_ENTER):
                        lvl = 0; selecting_diff = False
    ...
    def quit_game(self):
        ...
        if hasattr(self, 'game') and self.game is not None:
            ...
            if arm_obj and hasattr(arm_obj, 'arm'):
                try:
                    print("Parking arm at home position...")
                    arm_obj.arm.set_and_wait_goal_pos([2048, 1800, 1850, 1100, 2048, 2048])
